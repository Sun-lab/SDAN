...
def run_sdan():
   ...
   # ---------------- Gene Filtering ----------------
    ...
    tmp_sdan_dir = os.path.join(OUTROOT, "output")
    model_src = os.path.join(tmp_sdan_dir, f"model_{tag}.pth")
    train_s_src = os.path.join(tmp_sdan_dir, f"train_s_{tag}.npy")
    gene_list_src = os.path.join(tmp_sdan_dir, f"gene_list_{tag}.txt")

    model_path = os.path.join(out_dir, f"model_{tag}.pth")
    train_s_path = os.path.join(out_dir, f"train_s_{tag}.npy")
    gene_list_path = os.path.join(out_dir, f"gene_list_{tag}.txt")

    for src, dst in [
        (model_src, model_path),
        (train_s_src, train_s_path),
        (gene_list_src, gene_list_path),
    ]:
        if not os.path.exists(src):
            raise FileNotFoundError(f"[ERROR] Expected SDAN output not found: {src}")
        shutil.move(src, dst)

    model = torch.load(model_path, weights_only=False)

    train_s = torch.tensor(
        np.load(train_s_path),
        dtype=torch.float32,
    )

    gene_list = pd.Index(
        pd.read_csv(gene_list_path, header=None).squeeze()
    )
