...
from compute_loading_comparison import (
    DATASET_CONFIG,
    compute_sdan_connectivity_quantiles,
...
def build_auc_summary(
    ...
    for cell_type, weight in discover_sdan_runs(sdan_output):
        auc_frame = compute_sdan_test_separation_scores(
            repo_root=repo_root,
        ...
        connectivity_frame = compute_sdan_connectivity_quantiles(
            repo_root=repo_root,
            sdan_output=sdan_output,
            cell_type=cell_type,
            sdan_weight=weight,
            ...
            n_random=n_random,
            seed=seed,
        )[["program", "connectivity_quantile"]]
        frame = auc_frame.merge(connectivity_frame, on="program", how="left")
        frame["connectivity_quantile"] = frame["connectivity_quantile"].fillna(0.0)
        frames.append(frame)
