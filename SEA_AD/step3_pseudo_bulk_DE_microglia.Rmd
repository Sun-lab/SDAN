...
names(supertype) = gsub("-", "_", names(supertype))
...
cell_sub_types = unique(cell_info$Supertype)
cell_sub_types = gsub("-", "_", cell_sub_types)
fm1 = as.formula(sprintf("PC1 ~ -1 + %s", paste(cell_sub_types, collapse=" + ")))
fm2 = as.formula(sprintf("PC2 ~ -1 + %s", paste(cell_sub_types, collapse=" + ")))

summary(lm(fm1, data=colDat))
summary(lm(fm2, data=colDat))

tt1 = t.test(colDat$Micro_PVM_2 ~ colDat$condition)

ggplot(colDat, aes(x=condition, y=Micro_PVM_2, fill=condition)) +
  geom_boxplot() + ggtitle(sprintf("pval = %.2f", tt1$p.value))


ggplot(colDat, aes(x=PC1, y=Micro_PVM_2, color=condition)) +
  geom_point()

ggplot(colDat, aes(x=PC2, y=Micro_PVM_2, color=condition)) +
  geom_point()

ggplot(colDat, aes(x=PC2, y=Micro_PVM_2_3_SEAAD, color=condition)) +
  geom_point()
...
svs  = sva(dat, mod, mod0, n.sv=5)
dim(svs$sv)

cor(svs$sv, as.numeric(as.factor(colDat$condition)))

for(i in 1:ncol(svs$sv)){
  colDat[[paste0("SV", i)]] = svs$sv[,i]
}

dim(colDat)
flextable(colDat[1:2,])


mnames = c("Micro_PVM_1", "Micro_PVM_1_1_SEAAD", 
           "Micro_PVM_2", "Micro_PVM_2_1_SEAAD", "Micro_PVM_2_2_SEAAD", 
           "Micro_PVM_2_3_SEAAD", "Micro_PVM_3_SEAAD", "Micro_PVM_4_SEAAD")

ggcorrplot(cor(colDat[,..mnames], svs$sv, method="spearman"), lab = TRUE)

ggplot(colDat, aes(x=SV3, y=Micro_PVM_2_3_SEAAD, color=condition)) +
  geom_point()

ggplot(colDat, aes(x=SV3, y=Micro_PVM_2, color=condition)) +
  geom_point()
...
frm1 = sprintf("~log10Depth + Sex + age + PMI + RIN + %s + condition", 
               paste(paste0("SV", 1:n.sv), collapse=" + "))

dds2 <- DESeqDataSetFromMatrix(dat, colData=colDat, as.formula(frm1))
...
ggarrange(plotlist=plist, nrow=4, ncol=3)
```
