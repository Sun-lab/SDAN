...
table(t1)
gene_info[gene_symbol %in% names(t1)[t1 > 1],]

gene_info[sym_in_data == "HIST1H2BC", gene_symbol:="H2BC4"]
gene_info[sym_in_data == "HIST1H2BE", gene_symbol:="H2BC6"]
...
```{r}
gmtfile = list()
gmtfile[["reactome"]] = "../Annotation/c2.cp.reactome.v2022.1.Hs.symbols.gmt"
gmtfile[["go_bp"]]    = "../Annotation/c5.go.bp.v2022.1.Hs.symbols.gmt"
gmtfile[["immune"]]   = "../Annotation/c7.all.v2022.1.Hs.symbols.gmt"
gmtfile[["cell_type"]] = "../Annotation/c8.all.v2022.1.Hs.symbols.gmt"
...
gene_metrics = fread("data/CD8T_gene_info.tsv")
dim(gene_metrics)
gene_metrics[1:2,]

table(gene_info$sym_in_data %in% gene_metrics$gene)
gene_info = merge(gene_info, gene_metrics, by.x="sym_in_data", by.y="gene", 
...
for(k in 1:length(gene_sets)){
  ...
  pwf = nullp(genes, "hg38", "geneSymbol", bias.data = 1-gene_info$prct_zero)
...
saveRDS(goseq_res, "output/gene_set_enrichments.RDS")
```
