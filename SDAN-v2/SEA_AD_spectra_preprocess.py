...
os.makedirs(f"{d}output_v2_4k/", exist_ok=True)
...
# np.save(f"{d}output_v2_4k/gene_list_{args.cell_type}.npy", np.array(gene_list))
# print(f"[SAVED] gene_list_{args.cell_type}.npy ({len(gene_list)} genes)")

# Save both .npy and .txt versions for compatibility
np.save(f"{d}output_v2_4k/gene_list_{args.cell_type}.npy", np.array(gene_list))
pd.Series(gene_list).to_csv(f"{d}output_v2_4k/gene_list_{args.cell_type}.txt",
...
for name, ad in zip(["train", "val", "test"], [train_data, val_data, test_data]):
    ...
    subset.write(f"{d}output_v2_4k/{name}_{args.cell_type}_filtered.h5ad")
