"""
Su_2020_v3_scNET75.py — unified version for SDAN, sciRED, and scNET backends.
...
def run_sdan():
    ...
    out_dir = os.path.join(d, "output_v2", "SDAN")
    os.makedirs(out_dir, exist_ok=True)
...
def run_scired():
    ...
    out_dir = os.path.join(d, "output_v2", "sciRED")
    os.makedirs(out_dir, exist_ok=True)
    gene_list = get_de_gene_list(train_data, ["mild", "severe"], args.cell_type, out_dir=out_dir)
...
def run_scnet():
    ...
    out_dir = os.path.join(d, "output_v2", "scNET")
    os.makedirs(out_dir, exist_ok=True)
